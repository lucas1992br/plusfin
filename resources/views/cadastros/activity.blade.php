<x-aplicativo-layout>
                ...
                @slot('data')
                    @foreach ($activities as $item)
                        <tr>
                            ...
                            {{-- <td title="{{$item->atividade}}">{{$item->atividade ? 'Sim' : 'Não'}}</td> --}}
                            <td title="{{ $item->id }}">{{ $item->id }}</td>
                            <td title="{{ $item->nome }}">{{ $item->nome }}</td>
                            {{-- <td title="{{ $item->tipo }}">{{ $item->tipo }}</td> --}}
                            <td title="Ações">
                                <a role="button" class="delete-row-js" data-route="{{route('atividade.destroy', $item->id)}}">
                                    <i class="fa fa-trash _i text-danger"></i>
                                </a>
                                <a role="button" class="edit-row-js" data-route="{{route('atividade.show', $item->id)}}">
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="register-modal">Cadastro de Atividade</h5>
                    <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <form method="POST" action="{{ route('atividade.store') }}" class="needs-validation" novalidate>
                    <div class="modal-body">
                        ...
                        <div class="form-row">
                            ...
                            {{-- <div class="col-md-4 mb-3">
                                <label for="validationTooltip02">Atividade</label>
                                <input type="text" class="form-control" id="atividade" name='atividade' placeholder="Atividade" required>
                                <div class="invalid-tooltip">
    ...
    <div class="modal fade" id="edit-item-modal" tabindex="-1" role="dialog" aria-labelledby="update-modal" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="update-modal">Editar Atividade</h5>
                    <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <form id="edit-form" class="needs-validation" method="POST" novalidate>
                    <div class="modal-body">
                        ...
                        <div class="form-row">
                            ...
                            {{-- <div class="col-md-4 mb-3">
                                <label for="validationTooltip02">Atividade</label>
                                <input type="text" class="form-control" id="edit-atividade" name='atividade' placeholder="Atividade" value="" required>
                                <div class="invalid-tooltip">
                                    Atividade é obrigatório
                                </div>
                            </div>
        ...
        $('.edit-row-js').on('click', function(e) {
            ...
            $.ajax({
                ...
                success: (response, textStatus, xhr) => {
                    ...
                    $('#edit-atividade').val(response.atividade);
                    $('#edit-tipo').val(response.tipo);
                    $('#edit-form').attr('action', `atividade/${editItemId}`);
